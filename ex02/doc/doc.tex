...
$O(n m)$, wobei $n$ fuer die Anzahl primitiven der dimension $D - 1$ des konvexen polygons steht,
und $D$ fuer die dimensionalitaet unseres raumes.
Im 3D raum ist $n$ also die anzahl Ebenen aus denen die oberflaeche des konvezen polygons besteht.
$m$ ist die respektive anzahl des anderen polygons.
Clipping benoetigt linearen aufwand bezueglich $n m$,
weil man jede der $m$ flaechen gegen bei jeder der $n$ flaechen darauf pruefen muss,
in welchem halbraum sie liegt.

\subsection{5. Warum wird Backface-Culling am besten im kanonischen Sichtvolumen durchgeführt?}

Im kanonische Sichtvolumen weisen alle Blickstrahlen in dieselbe Richtung. Dadurch gibt es zwischen Normalenvektoren eines Polygons und den Blickstrahlen nur genau einen Winkel. Und dann weiter ...
TODO
